...
@Time    :   2019-08-26 14:34
 
@Desc    :   dynamic memory network 模型实现
...
class DynamicMemoryNetwork():
    ...
    def __init__(self, num_classes, learning_rate, batch_size, decay_steps, decay_rate, sequence_length, story_length,
        ...
        self.story = tf.placeholder(tf.int32, [None, self.story_length, self.sequence_length], name="story")    # 这里是干嘛的
        ...
        def input_model(self):
            ...
            self.story_embedding, hidden_state = tf.nn.dynamic_rnn(cell, story_embedding, dtype = tf.float32, scope ="input_model")

        def question_module(self):
            '''

            :param self: tokens of query [batch_size, sequence_length]
            ...
            cell = rnn.GRUCell(self.hidden_size)
            _, self.query_embedding = tf.nn.dynamic_rnn(cell, query_embedding, dtype = tf.float32, scope = "question_module")

        def episodic_memory_model(self):
            '''
            ...
            :return:
            '''
            candidate_inputs = tf.split(self.story_embedding, self.story_length,
                                        axis=1)  # a list. length is: story_length. each element is:[batch_size,1,embedding_size]
            candidate_list = [tf.squeeze(x, axis=1) for x in
                              candidate_inputs]  # a list. length is: story_length. each element is:[batch_size  ,embedding_size]
            ...
            for pass_number in range(self.num_pass):  # for each candidate sentence in the list,do loop.
                ...
                g = self.attention_mechanism_parallel(self.story_embedding, m_current, self.query_embedding,
                                                      pass_number)  # [batch_size,story_length]
                # 2.below is Memory Update Mechanism
                if self.use_gated_gru:  # use gated gru to update episode. this is default method.
                    g = tf.split(g, self.story_length,
                                 axis=1)  # a list. length is: sequence_length. each element is:[batch_size,1]
                    # 2.1 use gated-gru to update hidden state
                    for i, c_current in enumerate(candidate_list):
                        g_current = g[i]  # [batch_size,1]
                        h_current = self.gated_gru(c_current, h_current,
                                                   g_current)  # h_current:[batch_size,hidden_size]. g[i] represent score( a scalar) for current candidate sentence:c_current.
        ...
        def answer_module(self):
            ...
            :return:
            '''
            steps = self.sequence_length if self.sequence_length else 1
            a = self.m_T  # init hidden state
            y_pred = tf.zeros((self.batch_size,
                               self.hidden_size))  # TODO usually we will init this as a special token '<GO>', you can change this line by pass embedding of '<GO>' from outside.
            logits_list = []
            logits_return = None
        ...
        def gated_gru(self, c_current, h_previous, g_current):
            ...
            h_current = tf.multiply(g_current, h_candidate) + tf.multiply(1 - g_current,
                                                                          h_previous)  # [batch_size,hidden_size]
            return h_current
        ...
        def attention_mechanism_parallel(self, c_full, m, q, i):
            ...
            z = tf.concat(
                [c_full, m_tile, q_tile, c_q_elementwise, c_m_elementwise, c_q_minus, c_m_minus, c_w_q, c_w_m],
                2)  # [batch_size,story_length,hidden_size*9]
            # 2. two layer feed foward
            g = tf.layers.dense(z, self.hidden_size * 3,
                                activation=tf.nn.tanh)  # [batch_size,story_length,hidden_size*3]
        ...
        def x1Wx2_parallel(self, x1, x2, scope):
            ...
            with tf.variable_scope(scope):
                ...
                x1_w = tf.layers.dense(x1, self.story_length * self.hidden_size,
                                       use_bias=False)  # [self.hidden_size, story_length*self.hidden_size]
        ...
        def gru_cell(self, Xt, h_t_minus_1, variable_scope):
            ...
            with tf.variable_scope(variable_scope):
                ...
                z_t = tf.nn.sigmoid(tf.matmul(Xt, self.W_z) + tf.matmul(h_t_minus_1,
                                                                        self.U_z) + self.b_z)  # z_t:[batch_size,self.hidden_size]
                ...
                r_t = tf.nn.sigmoid(tf.matmul(Xt, self.W_r) + tf.matmul(h_t_minus_1,
                                                                        self.U_r) + self.b_r)  # r_t:[batch_size,self.hidden_size]
        ...
        def smoothing_cross_entropy(self, logits, labels, vocab_size,
            ...
            with tf.name_scope("smoothing_cross_entropy", [logits, labels]):
                ...
                normalizing = -(
                            confidence * tf.log(confidence) + tf.to_float(vocab_size - 1) * low_confidence * tf.log(
...
def train():
    ...
    story_length = 3
    dropout_keep_prob = 1
    model = DynamicMemoryNetwork(num_classes, learning_rate, batch_size, decay_steps, decay_rate,
                                 sequence_length,
