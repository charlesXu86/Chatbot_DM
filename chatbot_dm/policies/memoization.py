...
class MemoizationPolicy(Policy):
    """The policies that remembers exact examples of
        `max_history` turns from training stories.

        Since `slots` that are set some time in the past are
        preserved in all future feature vectors until they are set
        to None, this policies implicitly remembers and most importantly
        recalls examples in the context of the current dialogue
        longer than `max_history`.

        This policies is not supposed to be the only policies in an ensemble,
        ...
        no other policies can overrule it.
    ...
    def _standard_featurizer(max_history=None):
        # Memoization policies always uses MaxHistoryTrackerFeaturizer
    ...
    def train(
        ...
        **kwargs: Any
        ...
        """Trains the policies on given training trackers."""
        self.lookup = {}
    ...
    def load(cls, path: Text) -> "MemoizationPolicy":
        ...
        if os.path.isfile(memorized_file):
            ...
            logger.info(
                "Couldn't load memoization for policies. "
...
class AugmentedMemoizationPolicy(MemoizationPolicy):
    """The policies that remembers examples from training stories
        ...
        Since `slots` that are set some time in the past are
        preserved in all future feature vectors until they are set
        to None, this policies has a capability to recall the turns
