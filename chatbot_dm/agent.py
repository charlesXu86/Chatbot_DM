...
def _load_and_set_updated_model(
    ...
    try:
        ...
        logger.exception(
            "Failed to load policies and update agent. "
...
class Agent(object):
    ...
    def __init__(
        ...
        if not self._is_form_policy_present():
            raise InvalidDomain(
                ...
                "FormPolicy to your policies ensemble."
            )
    ...
    def toggle_memoization(self, activate: bool) -> None:
        ...
        If a memoization policies is present in the ensemble, this will toggle
        the prediction of that policies. When set to ``False`` the Memoization
        policies present in the policies ensemble will not make any predictions.
        Hence, the prediction result from the ensemble always needs to come
        from a different policies (e.g. ``KerasPolicy``). Useful to test
        prediction
        ...
        for p in self.policy_ensemble.policies:
            # explicitly ignore inheritance (e.g. augmented memoization policies)
    ...
    def continue_training(
        ...
        if not self.is_ready():
            raise AgentNotReady("Can't continue training without a policies ensemble.")
    ...
    def train(
        ...
        """Train the policies / policies ensemble using dialogue data from file.
        ...
        """
        if not self.is_ready():
            raise AgentNotReady("Can't train without a policies ensemble.")
        ...
        if kwargs.get("featurizer"):
            raise Exception(
                ...
                "to the policies configuration instead. More info "
        ...
        if (
            ...
            raise Exception(
                "Passing policies configuration parameters "
                "to `agent.train(...)` is not supported "
                "anymore. Specify parameters directly in the "
                "policies configuration instead. More info "
                "{}/core/migrations.html".format(LEGACY_DOCS_BASE_URL)
            )
    ...
    def persist(self, model_path: Text, dump_flattened_stories: bool = False) -> None:
        ...
        if not self.is_ready():
            raise AgentNotReady("Can't persist without a policies ensemble.")
    ...
    async def visualize(
        ...
        # largest value from any policies
    ...
    def _ensure_agent_is_ready(self) -> None:
        ...
        if not self.is_ready():
            raise AgentNotReady(
                ...
                "You need to set an interpreter, a policies "
    ...
    def _create_ensemble(
        ...
        if isinstance(policies, list):
            ...
            raise ValueError(
                ...
                "of type '{}', but should be policies, an array of "
                "policies, or a policies ensemble".format(passed_type)
            )
    ...
    def _is_form_policy_present(self) -> bool:
        """Check whether form policies is present and used."""
