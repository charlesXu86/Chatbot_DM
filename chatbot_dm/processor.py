...
@Desc    :   对话系统的核心处理模块，通过execution_action完成bot处理对话的流程
...
from chatbot_dm.interpreter import (
...
class MessageProcessor(object):
    ...
    async def execute_action(
        ...
        # we have a Tracker instance for each user
        # which maintains conversation state
        tracker = self._get_tracker(sender_id)       # 获取对话历史信息
        if tracker:
            action = self._get_action(action_name)
            await self._run_action(
                action, tracker, output_channel, nlg, policy, confidence       # 执行action
            )
