...
from chatbot_dm import constants, utils
from chatbot_dm.agent import load_agent, Agent
from chatbot_dm.channels import BUILTIN_CHANNELS, console
...
from chatbot_dm.util import AvailableEndpoints, configure_file_logging
...
def _create_single_channel(channel, credentials):
    from chatbot_dm.channels import BUILTIN_CHANNELS
...
def configure_app(
    ...
    if input_channels:
        chatbot_dm.channels.channel.register(input_channels, app, route=route)
    else:
        input_channels = []
...
if __name__ == "__main__":
    raise RuntimeError(
        "Calling `chatbot_dm.run` directly is no longer supported. "
